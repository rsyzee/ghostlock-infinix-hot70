...
/*static int write_selinux_policy_fix_script(void) {
  static const char fix_script[] =
    ...
    "cp $T $L && echo '[+] policy fix: policy load succeeded' && rm -f $T && exit 0 || echo '[!] policy fix: policy load failed' && rm -f $T && exit 1\n"
    "rm -f $T\n"
    "exit 1\n";
...
static void write_root_script(void) {
  ...
  const char *script =
    ...
    "load_policy /sys/fs/selinux/policy && diag '[+] early SELinux policy repair succeeded'\n"
